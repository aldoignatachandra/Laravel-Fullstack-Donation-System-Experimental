            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-slate-900">
                        {{ App\Helper\NumberHelper::formatIDR($totalDonationAmount) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl shadow-lg border border-slate-200/60 p-6 hover:shadow-xl transition-shadow">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-slate-900">
                        {{ $totalDonationCount > 0 ? App\Helper\NumberHelper::formatIDR($totalDonationAmount / $totalDonationCount) : App\Helper\NumberHelper::formatIDR(0) }}
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-3xl shadow-lg border border-slate-200/60 p-6 mb-8">
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('home') }}#campaigns"
                class="inline-flex items-center gap-2 px-5 py-2.5 bg-emerald-600 text-white rounded-2xl font-semibold text-sm hover:bg-emerald-700 transition shadow-md hover:shadow-lg">
    ...
    <div class="bg-white rounded-3xl shadow-lg border border-slate-200/60 p-6">
        ...
        @if ($recentDonations->count() > 0)
            <div class="space-y-4">
                @foreach ($recentDonations as $donation)
                    <a href="{{ route('campaign.show', $donation->campaign->slug) }}" wire:navigate
                       class="block p-4 rounded-2xl border border-slate-200 bg-slate-50 hover:bg-slate-100 hover:border-emerald-300 transition cursor-pointer">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <div class="flex flex-col items-end text-right gap-2">
                                ...
                                @php
                                    $statusColor = \App\Filament\Resources\Donations\Helpers\DonationHelper::getDonationStatusColor(
                                        $donation->status,
                                    );
                                    $statusLabel = \App\Filament\Resources\Donations\Helpers\DonationHelper::getDonationStatusLabel(
                                        $donation->status,
                                    );
        ...
        @else
            <div class="text-center py-16">
                ...
                <a href="{{ route('home') }}#campaigns"
                    class="inline-flex items-center gap-2 mt-4 px-5 py-2.5 bg-emerald-600 text-white rounded-2xl font-semibold text-sm hover:bg-emerald-700 transition">
                    Jelajahi Campaign
